...
7. **仓库访问权限**: `lcqxidian/SuperFlash` 当前为 private；未邀请的用户只获得仓库链接会看到 404，无法下载 DMG 或读取新用户文档。分发前需由仓库所有者决定邀请 collaborator、改为 public、创建 Release 或单独发送安装包

### 中优先级

5. **SettingsView 下载**: `downloadStdPeriphLib()` 把 300MB ZIP 全读进内存
6. **pkill 误杀**: `ScriptRunner` 的 `pkill -f` 可能结束同一用户的其他 IDE 调试会话
7. **STM32Family 枚举**: 只支持 F1/F4，Python 支持 14 个系列
8. **FloatingBallManager 内存泄漏**: 每次状态更新重建整个 rootView

### 低优先级

9. **进度检测**: 基于输出中的 ` $ ` 字符串，用户代码输出可能干扰
10. **浮动球状态双重同步**: AppState setter 和 ContentView onChange 都更新浮球
11. **报告中的硬编码路径**: Python 脚本引用 `~/.claude/skills/` 路径
...
- `gh repo view` 确认仓库 `visibility=PRIVATE`；未认证 HTTP 访问返回 404，文档已增加授权和分发方式说明。
...
- 因仓库当前为 private，“只分享链接”必须以接收方已获得 GitHub 权限为前提；Codex 不得自行改变仓库可见性或邀请成员。
- 依赖按 STM32/TI 与具体探针分支安装，不建议为单一用途一次性安装全部工具。
- 本次只创建和更新文档；未经用户明确授权，不执行 Git 写操作。
