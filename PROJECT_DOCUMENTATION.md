...
7. **SettingsView 下载**: `downloadStdPeriphLib()` 把 300MB ZIP 全读进内存
8. **pkill 误杀**: `ScriptRunner` 的 `pkill -f` 可能结束同一用户的其他 IDE 调试会话
9. **STM32Family 枚举**: 只支持 F1/F4，Python 支持 14 个系列
10. **FloatingBallManager 内存泄漏**: 每次状态更新重建整个 rootView

### 低优先级

11. **进度检测**: 基于输出中的 ` $ ` 字符串，用户代码输出可能干扰
12. **浮动球状态双重同步**: AppState setter 和 ContentView onChange 都更新浮球
13. **报告中的硬编码路径**: Python 脚本引用 `~/.claude/skills/` 路径
